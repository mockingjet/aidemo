...
export default {
  ...
  methods: {
      ...
      this.api
        ...
        .then(response => {
          ...
          for (var i = 0; i < this.output.image_circles.length; i++) {
            this.modified_cricles[i] = {
              x: this.output.image_circles[i].x,
              y: this.output.image_circles[i].y,
              type: ""
            };
          }
    ...
    */
    ...
    getColor(nth, type) {
      if (
        this.output.image_circles[nth].type === type ||
        type === "No Change"
      ) {
      ...
      this.api
        .post(this.url.getColor, this.modified_cricles)
        .then(response => {
          this.swal.close();
          var d = new Date();
          this.modified.image_file =
            response.data.image_file + "?time=" + d.getTime();
